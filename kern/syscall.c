...
sys_exofork(void)
{
	...
	struct Env *childenv;
	int ret;
	if((ret = env_alloc(&childenv, curenv->env_id))) {
	//	cprintf("error!:%e\n", ret);
		return ret;
	}
...
sys_env_set_status(envid_t envid, int status)
{
	...
	if (envid2env(envid, &todoenv, 1))
		return -E_BAD_ENV;
	if( (status != ENV_RUNNABLE) && (status != ENV_NOT_RUNNABLE))
		return -E_INVAL;
	todoenv->env_status = status;
	return 0;
...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
	// LAB 4: Your code here.
	struct Env *cenv;
	if (envid2env(envid, &cenv, 1))
		return -E_BAD_ENV;
	cenv->env_pgfault_upcall = func;
	return 0;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (!pp)
		return -E_NO_MEM;
	if (page_insert(cenv->env_pgdir, pp, va, perm) != 0) {
		page_free(pp);
		return -E_NO_MEM;
	}
	return 0;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (((uintptr_t)srcva < UTOP)) {
		...
		if (((perm & (PTE_U|PTE_P)) != (PTE_U|PTE_P)) || ((perm &~PTE_SYSCALL) != 0))
			return -E_INVAL;
		pte_t *cpte;
		if (!page_lookup(curenv->env_pgdir, srcva, &cpte))
			return -E_INVAL;
		if ((perm &PTE_W) && !(*cpte & PTE_W))
			return -E_INVAL;
		if ((uintptr_t)targetenv->env_ipc_dstva < UTOP) {
			if (sys_page_map(curenv->env_id, srcva, envid, targetenv->env_ipc_dstva, perm))
				return -E_NO_MEM;
			targetenv->env_ipc_perm = perm;
		}
		else targetenv->env_ipc_perm = 0;
	}
